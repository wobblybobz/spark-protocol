...
var Flasher = function Flasher(options) {
...
Flasher.prototype = extend(IFlasher.prototype, {
    ...
    startFlashFile: function startFlashFile(filename, client, onSuccess, onError) {
    ...
    startFlashBuffer: function startFlashBuffer(buffer, client, onSuccess, onError, onStarted) {
    ...
    setChunkSize: function setChunkSize(size) {
        this.chunk_size = size || Flasher.CHUNK_SIZE;
    },

    claimConnection: function claimConnection() {
    ...
    _nextStep: function _nextStep(data) {
        switch (this.stage) {
    ...
    prepare: function prepare() {
        ...
        if (this.fileBuffer) {
            ...
            var that = this;
            utilities.promiseStreamFile(this.filename).promise.then(function (readStream) {
    ...
    begin_update: function begin_update() {
        var that = this;
        var maxTries = 3;
    ...
    sendChunk: function sendChunk(chunkIndex) {
        ...
        if (this.chunk) {
            ...
            var writeCoapUri = function writeCoapUri(msg) {
    ...
    onChunkResponse: function onChunkResponse(msg) {
        if (this._protocolVersion > 0) {
    ...
    _sendAllChunks: function _sendAllChunks() {
        this.readNextChunk();
        while (this.chunk) {
    ...
    onAllChunksDone: function onAllChunksDone() {
     ...
     * @private
     */
    _waitForMissedChunks: function _waitForMissedChunks(wasAck) {
        if (this._protocolVersion <= 0) {
     ...
     * @private
     */
    _waitForMissedChunksDone: function _waitForMissedChunksDone() {
        if (this._chunkReceivedHandler) {
            this.client.removeListener("ChunkReceived", this._chunkReceivedHandler);
        }
        this._chunkReceivedHandler = null;
    ...
    onChunkMissed: function onChunkMissed(msg) {
        this._waitForMissedChunks();
    ...
    _resendChunk: function _resendChunk(idx) {
     ...
     * @param callback
     */
    failWatch: function failWatch(name, seconds, callback) {
        if (!this._timers) {
            this._timers = {};
        }
        if (!seconds) {
    ...
    _timers: null,
    clearWatch: function clearWatch(name) {
