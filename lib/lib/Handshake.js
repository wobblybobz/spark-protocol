...
Handshake.prototype = extend(IHandshake.prototype, {
    ...
    handshake: function handshake(client, onSuccess, onFail) {
        this.client = client;
    ...
    startGlobalTimeout: function startGlobalTimeout() {
        ...
        this._globalFailTimer = setTimeout(function () {
            this.handshakeFail("Handshake did not complete in " + Handshake.GLOBAL_TIMEOUT + " seconds");
        }.bind(this), Handshake.GLOBAL_TIMEOUT * 1000);
    },
    ...
    handshakeFail: function handshakeFail(msg) {
        //aww
        if (this.onFail) {
            this.onFail(msg);
        }
    ...
    onSocketData: function onSocketData(data) {
    ...
    send_nonce: function send_nonce() {
        this.stage++;
    ...
    //creates a session key, encrypts it using the core's public key, and sends it back
    send_sessionkey: function send_sessionkey() {
        var that = this;
