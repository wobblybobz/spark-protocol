...
module.exports = {
    ...
    getRandomBytes: function getRandomBytes(size, callback) {
    ...
    getRandomUINT16: function getRandomUINT16() {
    ...
    _serverKeys: null,
    setServerKeys: function setServerKeys(key) {
        if (ursa.isKey(key)) {
    ...
    getServerKeys: function getServerKeys() {
    ...
    _coreKeys: null,
    setCoreKeys: function setCoreKeys(key) {
        if (ursa.isKey(key)) {
    ...
    getCoreKeys: function getCoreKeys() {
    ...
    createPublicKey: function createPublicKey(key) {
        return ursa.createPublicKey(key);
    },

    encrypt: function encrypt(publicKey, data) {
        if (!publicKey) {
            return null;
        }
        if (!publicKey) {
    ...
    decrypt: function decrypt(privateKey, data) {
        if (!privateKey) {
    ...
    sign: function sign(privateKey, hash) {
        if (!privateKey) {
            privateKey = CryptoLib.getServerKeys();
        }
    ...
    verify: function verify(publicKey, hash, signature) {
        try {
     ...
     * @constructor
     */
    CreateAESCipher: function CreateAESCipher(sessionKey) {
     ...
     * @constructor
     */
    CreateAESDecipher: function CreateAESDecipher(sessionKey) {
        //The first 16 bytes (MSB first) will be the key, the next 16 bytes (MSB first) will be the initialization vector (IV), and the final 8 bytes (MSB first) will be the salt.
        var key = new Buffer(16); //just the key... +8); //key plus salt
        var iv = new Buffer(16); //initialization vector
    ...
    CreateAESCipherStream: function CreateAESCipherStream(sessionKey) {
        //The first 16 bytes (MSB first) will be the key, the next 16 bytes (MSB first) will be the initialization vector (IV), and the final 8 bytes (MSB first) will be the salt.
        var key = new Buffer(16); //just the key... +8); //key plus salt
        var iv = new Buffer(16); //initialization vector
     ...
     * @constructor
     */
    CreateAESDecipherStream: function CreateAESDecipherStream(sessionKey) {
        //The first 16 bytes (MSB first) will be the key, the next 16 bytes (MSB first) will be the initialization vector (IV), and the final 8 bytes (MSB first) will be the salt.
        var key = new Buffer(16); //just the key... +8); //key plus salt
        var iv = new Buffer(16); //initialization vector
    ...
    createHmacDigest: function createHmacDigest(ciphertext, key) {
        var hmac = crypto.createHmac('sha1', key);
        hmac.update(ciphertext);
        return hmac.digest();
    },

    loadServerPublicKey: function loadServerPublicKey(filename) {
    ...
    loadServerKeys: function loadServerKeys(filename, passFile, envVar) {
