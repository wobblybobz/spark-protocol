...
module.exports = {
     ...
     * @returns {Function}
     */
    raiseYourHandUrlGenerator: function raiseYourHandUrlGenerator(showSignal) {
        return function (msg) {
            var b = new Buffer(1);
    ...
    getRouteKey: function getRouteKey(code, path) {
        var uri = code + path;
     ...
     * @returns {*}
     */
    wrap: function wrap(name, id, params, data, token, onError) {
        var spec = StaticClass.Spec[name];
        if (!spec) {
            if (onError) {
     ...
     * @constructor
     */
    TranslateIntTypes: function TranslateIntTypes(varState) {
        if (!varState) {
            return varState;
        }
    ...
    getNameFromTypeInt: function getNameFromTypeInt(typeInt) {
        switch (typeInt) {
            case 1:
                return "bool";
            case 2:
    ...
    TryFromBinary: function TryFromBinary(buf, name) {
        var result = null;
        try {
    ...
    FromBinary: function FromBinary(buf, name) {
    ...
    ToBinary: function ToBinary(val, name, b) {
        name = name || (typeof val === 'undefined' ? 'undefined' : _typeof(val));
    ...
    buildArguments: function buildArguments(obj, args) {
        try {
    ...
    parseArguments: function parseArguments(args, desc) {
        try {
            ...
            for (var i = 0; i < desc.length; i++) {
                ...
                results.push(StaticClass.FromBinary(new Buffer(val, 'binary'), type));
            }
