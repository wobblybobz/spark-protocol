...
var utilities = {
    ...
    return new Promise(function (resolve, reject) {
      try {
        _fs2.default.exists(fileName, function (exists) {
          if (!exists) {
            _logger2.default.error('File: ' + fileName + ' doesn\'t exist.');
            reject();
          } else {
            resolve(_fs2.default.createReadStream(fileName));
          }
        });
      } catch (exception) {
        _logger2.default.error('promiseStreamFile: ' + exception);
  ...
  */
  convertDERtoPEM: function convertDERtoPEM(buffer) {
    ...
    try {
      var lines = ['-----BEGIN PUBLIC KEY-----'].concat(_toConsumableArray(bufferString.match(/.{1,64}/g) || []), ['-----END PUBLIC KEY-----']);
      return lines.join('\n');
    } catch (exception) {
