...
module.exports = {
    ...
    promiseDoFile: function promiseDoFile(filename, callback) {
    ...
    promiseStreamFile: function promiseStreamFile(filename) {
        var deferred = when.defer();
        try {
            fs.exists(filename, function (exists) {
                if (!exists) {
    ...
    bufferToHexString: function bufferToHexString(buf) {
        if (!buf || buf.length <= 0) {
            return null;
        }
    ...
    toHexString: function toHexString(val) {
    ...
    arrayContainsLower: function arrayContainsLower(arr, str) {
        if (arr && arr.length > 0) {
            str = str.toLowerCase();
     ...
     * @returns {*}
     */
    tryRequire: function tryRequire(filename) {
        try {
    ...
    tryMixin: function tryMixin(destObj, newObj) {
        try {
     ...
     * @returns {Array}
     */
    recursiveFindFiles: function recursiveFindFiles(dir, search, excludedDirs) {
     ...
     */
    promiseDoAllSequentially: function promiseDoAllSequentially(arr, handler) {
        var tmp = when.defer();
        var index = -1;
        var results = [];
    ...
    get_core_key: function get_core_key(coreid, callback) {
    ...
    * @returns {*}
    */
    convertDERtoPEM: function convertDERtoPEM(buf) {
        if (!buf || buf.length == 0) {
            return null;
        }
        var str;
