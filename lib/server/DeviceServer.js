...
var DeviceServer = function DeviceServer(options) {
...
DeviceServer.prototype = {
    ...
    init: function init() {
        ...
        this._severReadyPromise = new _promise2.default(function (resolve, reject) {
            _this._serverReady = resolve;
            _this._serverFailed = reject;
        });
        this.loadCoreData();
    },

    addCoreKey: function addCoreKey(coreid, public_key) {
        try {
    ...
    saveCoreData: function saveCoreData(coreid, attribs) {
        try {
            //assert basics
            attribs = attribs || {};
            //            attribs["coreID"] = coreid;

            var jsonStr = (0, _stringify2.default)(attribs, null, 2);
            if (!jsonStr) {
                return false;
            }
    ...
    getCore: function getCore(coreid) {
        return this._allCoresByID[coreid];
    },
    getCoreAttributes: function getCoreAttributes(coreid) {
    ...
    setCoreAttribute: function setCoreAttribute(coreid, name, value) {
        this._attribsByID[coreid] = this._attribsByID[coreid] || {};
        this._attribsByID[coreid][name] = value;
        this.saveCoreData(coreid, this._attribsByID[coreid]);
        return true;
    },
    getCoreByName: function getCoreByName(name) {
     ...
     * @returns {null}
     */
    getAllCoreIDs: function getAllCoreIDs() {
        return this._allIDs;
    },
     ...
     * @returns {null}
     */
    getAllCores: function getAllCores() {
        return this._allCoresByID;
    },

    /* publish special events */
    publishSpecialEvents: function publishSpecialEvents(name, data, coreid) {
    ...
    start: function start() {
        ...
        server.on('error', function () {
            logger.error("something blew up ", _arguments);
            _this2._serverFailed();
        });
        ...
        CryptoLib.loadServerKeys(settings.serverKeyFile, settings.serverKeyPassFile, settings.serverKeyPassEnvVar);
