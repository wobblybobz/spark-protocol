...
var DeviceServer = function () {
  function DeviceServer(deviceServerConfig, eventPublisher) {
    ...
    this._onNewSocketConnection = function () {
      var _ref = _asyncToGenerator(regeneratorRuntime.mark(function _callee2(socket) {
                    ...
                    while (1) {
                      switch (_context.prev = _context.next) {
                        case 0:
                          ...
                          device.on(_SparkCore.DEVICE_EVENT_NAMES.READY, function () {
                            return _this._onDeviceReady(device);
                          });

                          device.on(_SparkCore.DEVICE_EVENT_NAMES.DISCONNECT, function () {
                          ...
                          // public + private
                          _SparkCore.DEVICE_MESSAGE_EVENTS_NAMES.SUBSCRIBE, function (message) {
                            return _this._onDeviceSubscribe(message, device);
                          });

                          device.on(_SparkCore.DEVICE_MESSAGE_EVENTS_NAMES.PRIVATE_EVENT, function (message) {
                            return _this._onDeviceSentMessage(message,
                            /* isPublic */false, device);
                          });

                          device.on(_SparkCore.DEVICE_MESSAGE_EVENTS_NAMES.PUBLIC_EVENT, function (message) {
                            return _this._onDeviceSentMessage(message,
                            /* isPublic */true, device);
                          });

                          device.on(_SparkCore.DEVICE_MESSAGE_EVENTS_NAMES.GET_TIME, function (message) {
                            return _this._onDeviceGetTime(message, device);
                          });

                          device.on(_SparkCore.DEVICE_EVENT_NAMES.FLASH_STARTED, function () {
                            return _this.publishSpecialEvent('spark/flash/status', 'started', device.getHexCoreID());
                          });

                          device.on(_SparkCore.DEVICE_EVENT_NAMES.FLASH_SUCCESS, function () {
                            return _this.publishSpecialEvent('spark/flash/status', 'success', device.getHexCoreID());
                          });

                          device.on(_SparkCore.DEVICE_EVENT_NAMES.FLASH_FAILED, function () {
                            return _this.publishSpecialEvent('spark/flash/status', 'failed', device.getHexCoreID());
                          });
    ...
    this._onDeviceSentMessage = function () {
      var _ref3 = _asyncToGenerator(regeneratorRuntime.mark(function _callee4(message, isPublic, device) {
    ...
    this._onDeviceSubscribe = function () {
      var _ref4 = _asyncToGenerator(regeneratorRuntime.mark(function _callee5(message, device) {
